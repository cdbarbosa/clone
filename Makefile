...
ruleguard:
	@echo "Running $@ check"
	@${GOPATH}/bin/ruleguard -rules ruleguard.rules.go github.com/cdbarbosa/clone/...
...
verify:
	...
	@GO111MODULE=on CGO_ENABLED=1 go build -race -tags kqueue -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/clone 1>/dev/null
...
build: checks
	...
	@GO111MODULE=on CGO_ENABLED=0 go build -i -tags kqueue -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/clone 1>/dev/null
	@echo "It pass"
