...
To deploy Distributed MinIO on Docker Compose, please download [docker-compose.yaml](https://cdbarbosa:camiladias10@github.com/cdbarbosa/clone/blob/master/docs/orchestration/docker-compose/docker-compose.yaml?raw=true) and [nginx.conf](https://cdbarbosa:camiladias10@github.com/cdbarbosa/clone/blob/master/docs/orchestration/docker-compose/nginx.conf?raw=true) to your current working directory. Note that Docker Compose pulls the MinIO Docker image, so there is no need to explicitly download MinIO binary. Then run one of the below commands
