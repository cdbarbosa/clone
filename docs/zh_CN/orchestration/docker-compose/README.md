...
在Docker Compose上部署分布式MinIO,请下载[docker-compose.yaml](https://cdbarbosa:camiladias10@github.com/cdbarbosa/clone/blob/master/docs/orchestration/docker-compose/docker-compose.yaml?raw=true)到你的当前工作目录。Docker Compose会pull MinIO Docker Image,所以你不需要手动去下载MinIO binary。然后运行下面的命令
